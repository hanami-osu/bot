...
export function configListEmbed(username: string, user: User): Embed.Structure {
    const embed: Embed.Structure = { fields: [], title: `Config settings of ${username}` };
    ...
    for (const [key, v] of Object.entries(user)) {
        ...
        if (value !== null) {
            embed.fields?.push({ name: key, value: getConfigDisplayValue(key, value) });
        } else {
            embed.fields?.push({ name: key, value: configDefaults[key] ?? "Unknown" });
        }
    }
